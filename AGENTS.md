# Job Search Intelligence App — Project Context for Codex
...
2. **Match Scoring** (`/score/{role_id}`) — Sends JD + profile context to Codex API, returns match tier + rationale + gaps
3. **Interview Intel** (`/intel/{company}`) — Brave Search targeting Reddit, Levels.fyi, company hire pages; Codex summarizes into question frameworks
...
- Codex API summarizes results into:
...
- **Codex API for all inference** — match scoring, intel summarization, gap analysis
