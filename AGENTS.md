...
- **Per-role emails:** fire only for **Strong (≥80) and Perfect (≥90)** matches. Good Match (70–79) no longer emails for any company — Amazon's old 70 exception was removed 2026-08-24 (its internal-transfer lift already differentiates strong fits into 80+, so genuine strength still notifies). Bar is set in `notification_threshold()` / `COMPANY_NOTIFICATION_THRESHOLDS` in `notifications.py`. Email subject + header adapt to the role's actual match_tier.
