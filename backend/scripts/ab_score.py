#!/usr/bin/env python3
"""A/B scoring harness: Ox Alpha via OpenRouter (current) vs DeepSeek (candidate) on the same JDs.
...
Requires (in backend/.env or env): OPENROUTER_API_KEY, DEEPSEEK_API_KEY.
Endpoints + models configurable via OPENROUTER_BASE_URL / OPENROUTER_MODEL and
...
def _call_openrouter(system: str, user: str) -> str:
    # Route through ai_client so the OpenRouter branch applies the same model
    # mapping, reasoning-exclusion, and max_tokens scaling that prod uses.
    return ai_complete(None, system, user, max_tokens=1024)
...
def main(path: str) -> int:
    if "OPENROUTER_API_KEY" not in os.environ:
        print("Missing OPENROUTER_API_KEY in backend/.env")
        return 1
    ...
    print(f"OxAlpha: {os.getenv('OPENROUTER_MODEL', 'stealth/ox-alpha')} | DeepSeek: {DEEPSEEK_MODEL}\n")
    for i, role in enumerate(postings, 1):
        user = build_scoring_message(role, profile)
        try:
            ox_text = _call_openrouter(SCORING_SYSTEM_PROMPT, user)
            ds_text = _call_deepseek(SCORING_SYSTEM_PROMPT, user)
            c = _parse_score(ox_text)
        ...
        print(f"    OxAlpha  : {cs} {ct}  | ds=({c.get('dimension_scores', {})})")
        print(f"    DeepSeek : {ds} {dt}  | ds=({d.get('dimension_scores', {})})")
        if not same:
            print(f"    OxAlpha rationale  : {c.get('rationale','')[:300]}")
